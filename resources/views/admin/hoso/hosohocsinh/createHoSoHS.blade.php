               ...
               <table class="table table-striped table-bordered table-hover dataTable no-footer">
                  ...
                  <tbody id="dataDoituong">
                     <?php 
                        ...
                        ->get();
                        ?>
                     @foreach($qlhs_subject as $key => $val)
                     <tr>
                        <td class='text-center' style='vertical-align:middle;'>
                           <input onfocus="onFocus({{$val->subject_id}})" type='checkbox' value='{{$val->subject_id}}' id='checkboxactive_{{$val->subject_id}}' name='checkboxactive' class='checkboxactive_{{$key}}' onblur="onBlur({{$val->subject_id}})" />
                        </td>
                        <td style='vertical-align:middle;' class="activeCheckbox" id="checkboxfocus_{{$val->subject_id}}">
                           <label style='font-weight: 500' for='checkboxactive_{{$val->subject_id}}'>{{$val->subject_name}} <span style="color: red;font-size: 10px;font-style:italic" id="valid_{{$val->subject_id}}"></span></label>
                        </td>
                     </tr>
                     @endforeach
                  </tbody>
