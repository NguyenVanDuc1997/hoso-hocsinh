...
// open select2 dropdown on focus
    $(document).on('focus', '.select2-selection--single', function(e) {
        ...
        console.log(select2_open);

        select2_open.select2('open');
        //select2_open.select2('focus');
...
String.prototype.replaceAlls = function (find, replace) {
    var str = this;
...
function ConvertString(jsonDateString) {
    if (jsonDateString === null || jsonDateString === "-" || jsonDateString === undefined ) {
        return "-";
    }
...
function GET_INITIAL_NGHILC() {
    ...
    $("select.g_selectPaging").html('').selectpicker('refresh');
...
function SETUP_PAGING_NGHILC(data, callback) {
    ...
    if (StartRecord !== 0 || isRun === null) {
        $("select.g_selectPaging").selectpicker('val',StartRecord);
        return;
    }
    isRun = null;
    paging = 0;
    ...
    $("select.g_selectPaging").html(page_selector).selectpicker('refresh');;
    $("select.g_selectPaging").val(StartRecord).selectpicker('refresh');;
    //PHAN TRANG
    ...
    $("ul.g_clickedPaging").on('click', 'li.paging_item', (function () {
        ...
        $("select.g_selectPaging").selectpicker('val',rowCurrent);
...
GetFromServer = function (url, callbackSuc, callbackError, buttonLoading, nameFuction, pageName) {
    $('#' + buttonLoading).button('loading');
    if(nameFuction != null || nameFuction != ''){
        $("#loading").show();
    }
    $.ajax({
        type: "GET",
...
{
    var newObj = false; 
    jQuery.grep(object, function(obj) {
        if(parseInt(obj.subject_id) === parseInt(val) && obj.profile_subject_id != null){
            newObj = true;
        }
    });
...
function getDataObjects(object,val) 
{
    var newObj = false; 
    jQuery.grep(object, function(obj) {
        if(parseInt(obj.subject_id) === parseInt(val) && obj.profile_subject_id != null){
