...
function loadComboxTruongHoc(id,callback,idchoise = null,choose = 0) {
        //loading();
            $.ajax({
                type: "GET",
                      ...
                      //  for (var j = 0; j < datakhoi.length; j++) {
                          //  html_show +="<optgroup label='"+datakhoi[j].unit_name+"'>";
                                if(dataget.length > 0){
                                    ...
                                    html_show += "<option value=''>-- Chưa có trường học --</option>";
                                }
                         //   html_show +="</optgroup>"
                      //  }
                        $('#'+id).html(html_show);
...
//----------------------------------------------------------Send Thẩm định--------------------------------------------------------------
    ...
    function deleteTonghop( report_name) {
            ...
            success: function(datas) {
                ...
                if (dataget.length > 0) {
                    for (var i = 0; i < dataget.length; i++) {
                        ...
                        if (dataget[i].MGHP_name !== null && dataget[i].MGHP_name !== "") {
                            report_name = dataget[i].MGHP_name;
                        }
                        else if (dataget[i].CPHT_name !== null && dataget[i].CPHT_name !== "") {
                            report_name = dataget[i].CPHT_name;
                        }
                        else if (dataget[i].HTAT_name !== null && dataget[i].HTAT_name !== "") {
                            report_name = dataget[i].HTAT_name;
                        }
                        else if (dataget[i].HTBT_name !== null && dataget[i].HTBT_name !== "") {
                            report_name = dataget[i].HTBT_name;
                        }
                        else if (dataget[i].HSKT_name !== null && dataget[i].HSKT_name !== "") {
                            report_name = dataget[i].HSKT_name;
                        }
                        else if (dataget[i].NGNA_name !== null && dataget[i].NGNA_name !== "") {
                            report_name = dataget[i].NGNA_name;
                        }
                        else if (dataget[i].HTATHS_name !== null && dataget[i].HTATHS_name !== "") {
                            report_name = dataget[i].HTATHS_name;
                        }
                        else if (dataget[i].HSDTTS_name !== null && dataget[i].HSDTTS_name !== "") {
                            report_name = dataget[i].HSDTTS_name;
                        }
                        else if (dataget[i].HBHSDTNT_name !== null && dataget[i].HBHSDTNT_name !== "") {
                            report_name = dataget[i].HBHSDTNT_name;
                        }
                        ...
                        html_show += "<td class='text-center'>";
                            if(check_Permission_Feature('5')){
                                ...
                                //     && parseInt(dataget[i].HTATHS_status) === 0 && parseInt(dataget[i].HSDTTS_status) === 0 && parseInt(dataget[i].HBHSDTNT_status) === 0){
                                    ...
                                    if(check_Permission_Feature('3')){
                                        html_show += "<button data='' onclick='deleteTonghop(\""+report_name+"\");' class='btn btn-danger btn-xs editor_remove'><i class='glyphicon glyphicon-remove'></i> Xóa</button>";
                                    }
