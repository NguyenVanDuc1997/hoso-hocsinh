...
class HethongController extends Controller
{
    ...
    public function loadAllUser(Request $req){
      try {
        ...
        $count = DB::table('users as  u1');
        if(Auth::user()->truong_id !=null && Auth::user()->truong_id != 0 && Auth::user()->truong_id != '0'){
            $count = $count->whereIn('u1.truong_id',explode('-',Auth::user()->truong_id))->where('u1.level',Auth::user()->level);
        }
        $json['totalRows'] = $count->count();
        ...
        if(Auth::user()->truong_id !=null && Auth::user()->truong_id != 0 && Auth::user()->truong_id != '0'){
            $data = $data->whereIn('u1.truong_id',explode('-',Auth::user()->truong_id))->where('u1.level',Auth::user()->level);
        }
    ...
    public function insertUser(Request $req){
      ...
      try{
        ...
        $phongban_id = $req->input("phongban_id") != '' ? $req->input("phongban_id") : null;
        $count_s = DB::table('qlhs_schools')->where('schools_active','=',1)->select('schools_id')->count();
        $truong_id = '0';
        if($count_s == count($req->input("truong_id"))){
            $truong_id = '0';
        }else{
            $truong_id = implode($req->input("truong_id"),'-');
        }
    ...
    public function deleteUser($id){
      ...
      try{
        ...
        $json['success'] = "Xóa người dùng thành công";
