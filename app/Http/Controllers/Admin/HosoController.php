...
class HosoController extends Controller
{
    public function danhsachdenghi(){
        ...
        $congvan = '';
        return view('admin.hoso.danhsach.danhsachdenghi',['school' => $school,'congvan' =>$congvan]);
    }
    public function viewlist(){
        $list = DB::table('users')->paginate(5);
        return view('admin.hoso.hosohocsinh.danhsach',['list'=>$list]);
    }
    public function changeSubject(){
        return view('admin.hoso.hosohocsinh.doituong',['s_id' =>'','c_id' =>'','p_id' => '']);
    }
    ...
    public function ViewPheDuyetNew(){
        $school = Auth::user()->truong_id;
        $congvan = '';
        return view('admin.hoso.thamdinhdanhsach.listingnew',['school' => $school,'congvan' =>$congvan]);
    }
    public function getViewPheDuyetNew($school,$congvan){
        return view('admin.hoso.thamdinhdanhsach.listingnew',['school' => $school,'congvan' =>$congvan]);
    }
...
//Helper get count data unit Export Excel-----------------------------------------------------------------------------------------------------------
    ...
    public function loadCongVanByTruongChoDuyet(Request $rq){
        try {
            $json = [];
            ...
            $report = DB::table('qlhs_report')->leftJoin('qlhs_schools','report_school_id','schools_id')->where('qlhs_report.report_status', '=', 0);
            
            if($rq->id_school != null && $rq->id_school != '' && $rq->id_school != 0){
                        $report = $report->where('report_school_id',$rq->id_school);
            }else if($rq->id_school == null || $rq->id_school == '' || count(explode('-',Auth::user()->truong_id)) == 1 ){
                if(Auth::user()->truong_id != 0){
                    $report = $report->whereIn('report_school_id',explode('-',Auth::user()->truong_id));    
                }
            }

            if (!is_null($rq->keyword) && !empty($rq->keyword)) {
                $report = $report->where('qlhs_report.name', 'LIKE','%'.$rq->keyword.'%')->orWhere('schools_name', 'LIKE','%'.$rq->keyword.'%');
            }

            // if ($levelUser == 1) {
            //     $report = $report->where('report_waiting', '>', 0)
            //         ->select('qlhs_report.name as report_name', 'schools_name', 'report_school_id as report_id_truong', DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted')
            //         ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted');
            // }
            // else 
            if ($levelUser == 2) {
                $report = $report->where('report_waiting_phongGD', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongGD as report_waiting', 'report_approved_phongGD as report_approved', 'report_reverted_phongGD as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongGD', 'report_approved_phongGD', 'report_reverted_phongGD');
            }
            else if ($levelUser == 3) {
                $report = $report->where('report_waiting_phongTC', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongTC as report_waiting', 'report_approved_phongTC as report_approved', 'report_reverted_phongTC as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongTC', 'report_approved_phongTC', 'report_reverted_phongTC');
            }
            else if ($levelUser == 4) {
                $report = $report->where('report_waiting_So', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_So as report_waiting', 'report_approved_So as report_approved', 'report_reverted_So as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_So', 'report_approved_So', 'report_reverted_So');
            }
            
            $json['startRecord'] = ($rq->start);
            $json['numRows'] = $rq->limit;
            $json['totalRows'] = count($report->get());
            $json['data'] = $report->orderBy('report_startdate','desc')->orderBy('schools_name')->skip($rq->start*$rq->limit)->take($rq->limit)->get();
    ...
    public function loadCongVanByTruongDaDuyet(Request $rq){
        try {
            $json = [];
            ...
            // if ($levelUser == 1) {
            //     $report = $report->where('qlhs_report.report_approved', '>', 0)
            //         ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted')
            //         ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted');
            // }
            // else 
            if ($levelUser == 1 || $levelUser == 2) {
                $report = $report->where('report_approved_phongGD', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongGD as report_waiting', 'report_approved_phongGD as report_approved', 'report_reverted_phongGD as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongGD', 'report_approved_phongGD', 'report_reverted_phongGD');
            }
            else if ($levelUser == 3) {
                $report = $report->where('report_approved_phongTC', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongTC as report_waiting', 'report_approved_phongTC as report_approved', 'report_reverted_phongTC as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongTC', 'report_approved_phongTC', 'report_reverted_phongTC');
            }
            else if ($levelUser == 4) {
                $report = $report->where('report_approved_So', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_So as report_waiting', 'report_approved_So as report_approved', 'report_reverted_So as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_So', 'report_approved_So', 'report_reverted_So');
            }
    ...
    public function loadCongVanTraLai(Request $rq){
        try {
            $json = [];
            ...
            // if ($levelUser == 1) {
            //     $report = $report->where('report_reverted', '>', 0)
            //         ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted')
            //         ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting', 'qlhs_report.report_approved', 'report_reverted');
            // }
            // else 
            if ($levelUser == 1 || $levelUser == 2) {
                $report = $report->where('report_reverted_phongGD', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongGD as report_waiting', 'report_approved_phongGD as report_approved', 'report_reverted_phongGD as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongGD', 'report_approved_phongGD', 'report_reverted_phongGD');
            }
            else if ($levelUser == 3) {
                $report = $report->where('report_reverted_phongTC', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_phongTC as report_waiting', 'report_approved_phongTC as report_approved', 'report_reverted_phongTC as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_phongTC', 'report_approved_phongTC', 'report_reverted_phongTC');
            }
            else if ($levelUser == 4) {
                $report = $report->where('report_reverted_So', '>', 0)
                    ->select('qlhs_report.name as report_name','schools_name','report_school_id as report_id_truong',DB::raw('MAX(report_startdate) as report_date'), 'qlhs_report.report_enddate', 'report_waiting_So as report_waiting', 'report_approved_So as report_approved', 'report_reverted_So as report_reverted')
                    ->groupBy('qlhs_report.name', 'schools_name', 'report_school_id', 'qlhs_report.report_enddate', 'report_waiting_So', 'report_approved_So', 'report_reverted_So');
            }
    ...
    public function loadCongVan(Request $rq){
        try {
            $json = [];
            ...
            $report = $report->select('name', 'schools_name', 'report_school_id', 'report_startdate', 'report_enddate', 'report_approved', 'report_waiting_phongGD', 'report_approved_phongGD', 'report_reverted_phongGD', 'report_waiting_phongTC', 'report_approved_phongTC', 'report_reverted_phongTC', 'report_waiting_So', 'report_approved_So', 'report_reverted_So', 'report_reload_phongGD', 'report_reload_phongTC', 'report_reload_truong');
