...
class lapcongvanCapTruong{

	public static function setMoneyProfile($school_id,$year)
    {
        try {
            ...
            // Lấy học sinh trong bảng tổng hợp tính tiền - theo trường và theo năm học cần lập
            $getProfileBySchool = DB::table('qlhs_tonghopchedo')
                    ->join('qlhs_profile', 'profile_id','qlhs_thcd_profile_id')
                    ->join('qlhs_schools', 'schools_id','profile_school_id')
                    ->join('qlhs_class', 'class_id','profile_class_id')
                    ->join('qlhs_unit', 'unit_id','class_unit_id')
                    ->join('qlhs_hosobaocao_trangthai_Truong', 'rppst_profile_id', 'profile_id')
                    ->where('qlhs_thcd_school_id', $school_id)
                    ->where('qlhs_thcd_nam', $year)
                    ->where('rppst_year', $year)
                    ->where(function($q){
                        $q->orWhere(function($q){
                            $q->whereNotNull('rppst_Status_HK1')
                            ->where('rppst_Status_HK1',1);
                        })
                        ->orWhere(function($q){
                            $q->whereNotNull('rppst_Status_HK2')
                            ->where('rppst_Status_HK2',1);
                        });
                    })
                    ->select('profile_id as profileID','class_unit_id')->get();

            if (!is_null($getProfileBySchool) && !empty($getProfileBySchool) && count($getProfileBySchool) > 0) {
                foreach ($getProfileBySchool as $valuePID) {
                    $DTMien100_1 = 0;
                    $DTMien100_2 = 0; //DT100
                    $DTMien100_3 = 0;
                    $DTMien100_4 = 0;
                ...
                //-------------------------------------------------------------------MGHP------------------------------------------------------------
                    if ($chedoMGHP > 0 && $valuePID->class_unit_id != 2) {
                        ...
                        $dataMGHP1 = DB::table('qlhs_profile')
                            ...
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_MGHP_HK2 > 0 OR cur.rppst_Status_MGHP > 0 OR cur.rppst_Status_MGHP_HK2 > 0)'))
                        ...
                        $dataMGHP3 = DB::table('qlhs_profile')
                            ...
                            ->whereIn('profile_subject_subject_id', [28, 35, 36, 73, 38, 39, 40, 41, 100, 101])
                            ->where('profile_id', $valuePID->profileID)
                ...
                //-------------------------------------------------------------------HTAT------------------------------------------------------------
                    if ($chedoHTAT > 0 && $valuePID->class_unit_id == 1) {
                        $dataHTAT1 = DB::table('qlhs_profile')
                            ...
                            ->where('profile_year', '<', $year.'-06-01')
                            ->whereIn('profile_subject_subject_id', [73, 26, 34, 28, 41, 74, 100])
                            ->where('profile_id', '=', DB::raw($valuePID->profileID.' AND (old.rppst_Status_HTAT_HK2 > 0 OR cur.rppst_Status_HTAT > 0 OR cur.rppst_Status_HTAT_HK2 > 0)'))
                            ...
                            ->select(
                                ...
                                DB::raw('MAX(CASE when profile_subject_subject_id in (26, 34) then 1 else 0 END) DT1'),
                                ...
                                DB::raw("MAX(CASE when level_old <> '' and qlhs_profile.profile_year < '".$year."-06-01' and profile_subject_subject_id in (73,26,34,28,41,74) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$year."-01-01')) then 1 else 0 END) 'HK2_old'"),
                                DB::raw("MAX(CASE when level_cur <> '' and qlhs_profile.profile_year < '".$year."-12-31' and profile_subject_subject_id in (73,26,34,28,41,74) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".$year."-05-31')) then 1 else 0 END) 'HK1_cur'"),
                                DB::raw("MAX(CASE when level_cur <> '' and qlhs_profile.profile_year < '".($year +1)."-06-01' and profile_subject_subject_id in (73,26,34,28,41,74) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$year+1)."-01-01')) then 1 else 0 END) 'HK2_cur'"),
                                DB::raw("MAX(CASE when level_new <> '' and qlhs_profile.profile_year < '".($year +1)."-12-31' and profile_subject_subject_id in (73,26,34,28,41,74) and (profile_leaveschool_date is null or (profile_leaveschool_date >= '".((int)$year+1)."-05-31')) then 1 else 0 END) 'HK1_new'"));
                ...
                //-------------------------------------------------------------------HSDTTS----------------------------------------------------------
                    if ($chedoHSDTTS > 0 && $valuePID->class_unit_id == 33) {
                        $getDataTypeA = DB::table('qlhs_profile')
                            ...
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_HSDTTS_HK2 > 0 OR cur.rppst_Status_HSDTTS > 0 OR cur.rppst_Status_HSDTTS_HK2 > 0)'))
                ...
                //-------------------------------------------------------------------CPHT------------------------------------------------------------
                    if ($chedoCPHT > 0) {
                        $dataCPHT1 = DB::table('qlhs_profile')
                            ...
                            ->where('profile_year', '<', $year.'-06-01')
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_CPHT_HK2 > 0 OR cur.rppst_Status_CPHT > 0 OR cur.rppst_Status_CPHT_HK2 > 0)'))
                ...
                //-------------------------------------------------------------------HSKT------------------------------------------------------------
                    if ($chedoHSKT > 0) {
                        $getDataTypeA = DB::table('qlhs_profile')
                            ...
                            ->where('qlhs_profile.profile_year', '<', $year.'-06-01')
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_HSKT_HB_HK2 > 0 OR old.rppst_Status_HSKT_DDHT_HK2 > 0 OR cur.rppst_Status_HSKT_HB > 0 OR cur.rppst_Status_HSKT_DDHT > 0 OR cur.rppst_Status_HSKT_HB_HK2 > 0 OR cur.rppst_Status_HSKT_DDHT_HK2 > 0)'))
                ...
                //-------------------------------------------------------------------HTATHS----------------------------------------------------------
                    if ($chedoHTATHS > 0 && $getSchoolType->type_his_type_id == 2) {
                        $getDataTypeA = DB::table('qlhs_profile')
                            ...
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_HTATHS_HK2 > 0 OR cur.rppst_Status_HTATHS > 0 OR cur.rppst_Status_HTATHS_HK2 > 0)'))
                            ->where('qlhs_profile.profile_year', '<', $year.'-06-01')
                ...
                //-------------------------------------------------------------------HBHSDTNT--------------------------------------------------------
                    if ($chedoHBHSDTNT > 0 && $getSchoolType->type_his_type_id == 4) {
                        $getDataTypeA = DB::table('qlhs_profile')
                            ...
                            ->where('profile_id', DB::raw($valuePID->profileID.' AND (old.rppst_Status_HBHSDTNT_HK2 > 0 OR cur.rppst_Status_HBHSDTNT > 0 OR cur.rppst_Status_HBHSDTNT_HK2 > 0)'))
                            ->where('qlhs_profile.profile_year', '<', $year.'-06-01')
    ...
    public static function demo($school_id,$year,$profile_id,$type_school,$class_unit_id){
