...
$(function () {
  ...
  $('#btnPhanLoai').click(function(){
    ...
    $("#drSiteLevel").change(function(){
      loadComboXaPhuong($(this).val());
    //  getSitebyLevel($(this).val(), null);
      if (parseInt($(this).val()) == 3) {
        $('#dvSiteType').show();
      }
      else { $('#dvSiteType').hide(); }
    });
    ...
    $('#using_json_2').on("changed.jstree", function (e, data) {
        ...
        PostToServer('/danh-muc/xaphuong/getXaPhuongbyID',{ SITEID: currentIdSelected },function(results){
          var level = 0;
              var item = '';
              $('#txtSiteID').val(results[0]['site_id']);
              // $('#txtSiteCode').val(results[0]['site_code']);
              $('#txtSiteName').val(results[0]['site_name']);
              $('#drSiteLevel').selectpicker('val',results[0]['site_level']);
              $('#drSiteActive').selectpicker('val',results[0]['site_active']);

              if (parseInt(results[0]['site_level']) === 3) {
                $('#dvSiteType').show();
                $("#drSiteType").selectpicker('deselectAll');
                if(results[0]['site_type'] !== null && results[0]['site_type'] !== ""){
                  
                  $("#drSiteType").selectpicker('val',results[0]['site_type']);
                }
                $("#drSiteType").selectpicker("refresh");
              }
              else { 
                $('#dvSiteType').hide(); 
              }
          ...
          console.log("using_json_2 changed.jstree");
          console.log(results);
        },"","","");
    });
            ...
            <input type="hidden" class="form-control" id="txtSiteID">    
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <select id="drSiteLevel" class="form-control selectpicker" data-live-search="true">
                      <option value="0" selected="selected">-- Chọn cấp hành chính --</option>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <select id="drSiteParents" class="form-control selectpicker" data-live-search="true">
                      <option value='0' selected="selected">-- Chọn cấp trực thuộc --</option>
                    </select>
                  </div>
                </div>
                <div class="form-group" id="dvSiteType">
                  <label class="col-sm-3 control-label">Loại xã</label>
                  <div class="col-sm-9">
                    <select id="drSiteType" multiple="multiple"  class="selectpicker form-control" data-actions-box="true" data-live-search="true" multiple data-selected-text-format="count">
                      <option value="1">Xã thuộc vùng cao</option>
                      <option value="2">Xã thuộc vùng đặc biệt khó khăn</option>
                    </select>
                    <!-- <img src="../../images/Image_valid.png" id="imgValidName"><label id="lblValidName" class="valid"></label> -->
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <select id="drSiteActive" class="form-control selectpicker" data-live-search="true">
                 ...
                 <div class="modal-footer">
                        <div class="row text-center">
                            <button type="button" class="btn btn-primary" data-loading-text="Đang tải dữ liệu" id="btnInsertSite">Lưu</button>
