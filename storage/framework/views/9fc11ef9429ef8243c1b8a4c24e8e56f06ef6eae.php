...
<script type="text/javascript">
  $(function () {
    ...
    loaddutoannguoinauan($('#sltYear').val(),$('#sltTruong').val());
    $('#sltYear').change(function(){
        html_view = "";
        var _value = $(this).val();
        if(_value != ""){
          loaddutoan(_value,$('#sltTruong').val());
          loaddutoannguoinauan(_value,$('#sltTruong').val());
        }
    });
    ...
    permission(function(){
      var html_view  = '';
      var html_view_header  = '<b>Quản lý dự toán chi trả </b> / Danh sách dự toán';
  ...
  function loaddutoannguoinauan(year,school){
      GetFromServer('/du-toan-chi-tra/dutoan/loadngna?year='+year+'&school='+school,function(data){
        html_header = '';
        html_header += '<tr class="success">';
        html_header += '<th class="text-center" rowspan="2" style="vertical-align:middle">Năm học</th>';
        html_header += '<th class="text-center" rowspan="2" style="vertical-align:middle">Tổng học sinh</th>';
        html_header += '<th class="text-center" colspan="3" style="vertical-align:middle">Số lượng nhân viên</th>';
        html_header += '<th class="text-center" colspan="3" style="vertical-align:middle">Dự toán kinh phí</th>';
        html_header += '</tr>';
        html_header += '<tr class="success">';
        html_header += '<th class="text-center" style="vertical-align:middle">TW</th>';
        html_header += '<th class="text-center" style="vertical-align:middle">ĐP</th>';
        html_header += '<th class="text-center" style="vertical-align:middle">HĐ 68</th>';
        html_header += '<th class="text-center" style="vertical-align:middle">TW</th>';
        html_header += '<th class="text-center" style="vertical-align:middle">ĐP</th>';
        html_header += '<th class="text-center" style="vertical-align:middle">Tổng</th></tr>';
        html_body_detail = '';
        if(data.length > 0){
          var TW = 0;
          var DP = 0;
          ...
          for (var i = 0; i < data.length; i++) {
              var val = data[i].NGNA_HK.split('-');
              html_body_detail += "<tr>";
              if(val[0] == 'HK1'){
                  html_body_detail += "<td class='text-center'>Từ 9/"+val[1]+" đến 12/"+val[1]+"</td>";
              }else{
                  html_body_detail += "<td class='text-center'>Từ 1/"+(parseInt(val[1]) + 1)+" đến 5/"+(parseInt(val[1]) + 1)+"</td>";
              }
              
              html_body_detail += "<td class='text-right'>"+convertNumber(data[i].NGNA_amount)+"</td>";
              html_body_detail += "<td class='text-right'>"+convertNumber(data[i].NGNA_TW)+"</td>";
              html_body_detail += "<td class='text-right'>"+convertNumber(data[i].NGNA_DP)+"</td>";
              html_body_detail += "<td class='text-right'>"+convertNumber(data[i].NGNA_68)+"</td>";
              html_body_detail += "<td class='text-right'>"+formatter(data[i].TW)+"</td>";
              html_body_detail += "<td class='text-right'>"+formatter(data[i].DP)+"</td>";
              html_body_detail += "<td class='text-right'>"+formatter(convertNumber(data[i].TW) + convertNumber(data[i].DP))+"</td>";
              TW += convertNumber(data[i].TW);
              DP += convertNumber(data[i].DP);
              Tong += (convertNumber(data[i].TW) + convertNumber(data[i].DP));
          }
          html_body_detail += "<tr>";
          html_body_detail += "<td class='text-center'>Dự toán</td>";
          html_body_detail += "<td colspan='4' class='text-right'>-</td>";
          html_body_detail += "<td class='text-right'>"+formatter(TW)+"</td>";
          html_body_detail += "<td class='text-right'>"+formatter(DP)+"</td>";
          html_body_detail += "<td class='text-right'>"+formatter(Tong)+"</td>";
          html_body_detail += "<tr>";
          $('#dataHeaderDetailNGNA').html(html_header);
          $('#dataBodyDetailNGNA').html(html_body_detail);
        }
      },function(result){
          console.log("loaddutoannguoinauan");
          console.log(result);
      },"","","");
  }
    ...
    <div class="box box-info">
            ...
            <div class="box box-primary">
                ...
                <div class="box-body" style="font-size:12px;overflow: auto ; max-width: 100%">
                    ...
                    <hr>
                    <table class="table table-striped table-bordered table-hover dataTable no-footer">
                      <thead id="dataHeaderDetailNGNA">
                 
                      </thead>
                        <tbody id="dataBodyDetailNGNA">                     
                        </tbody>
                    </table>
                </div>       
            </div>
